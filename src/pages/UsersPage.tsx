...
function UserDialog({
  ...
  roleOptions: { name: string; displayName: string }[];
  onClose: () => void;
      ...
      >
        ...
        <div className="dialog-body">
          ...
          >
            ...
            <span>
              此動作無法復原。將永久刪除 <strong>{user.username}</strong>{" "}
              的所有資料、角色指派與稽核引用。
            </span>
          </div>
        ...
        <div className="dialog-footer">
          ...
          <button
            ...
            disabled={typeMatch !== user.username}
            onClick={onConfirm}
          >
            我了解，永久刪除
          </button>
...
export default function UsersPage() {
  ...
  const [confirmDel, setConfirmDel] = useState<UserListItem | null>(null);
  ...
  function save(form: UserForm) {
    if (dialog.user) {
      ...
      updateUser.mutate({ id: dialog.user.id, request: req });
    } else {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-desc">
            建立、停用與調整本地或 LDAP 使用者帳號。共 {users.length} 個帳號。
          </p>
        </div>
        <div className="page-actions">
          <button className="btn btn-secondary">
            <Icon name="refresh" size={13} /> 重新整理
          </button>
          <button
      ...
      >
        <div className="card">
          <div className="stat-label">總帳號數</div>
        ...
        <div className="card">
          <div className="stat-label">LDAP 帳號</div>
          <div className="stat-value">
            {users.filter((u) => u.authType === "ldap").length}
          </div>
        </div>
        <div className="card">
          <div className="stat-label">本機帳號</div>
          <div className="stat-value">
      ...
      <div className="card" style={{ padding: 0 }}>
        <div className="toolbar">
            ...
            <div className="segmented">
              {FILTERS.map((f) => (
                <div
                  key={f.id}
                  className={
                    "segmented-item" + (filter === f.id ? " active" : "")
                  }
          ...
          <div style={{ fontSize: 11, color: "var(--color-text-tertiary)" }}>
            顯示 {filtered.length} / {users.length}
          </div>
        </div>
                ...
                <tr key={u.id}>
                  ...
                  <td>
                    <span
                      className={
                        "pill " +
                        (u.authType === "ldap" ? "pill-info" : "pill-neutral")
                      }
                    >
                  ...
                  <td>
                    <span
                      className={
                        "pill " + (u.isActive ? "pill-success" : "pill-neutral")
                      }
                    >
                      <span
                        className="pill-dot"
                        style={{
                          background: u.isActive ? "#1D9E75" : "#A49DAE",
                        }}
                      />
                      {u.isActive ? "啟用" : "停用"}
                    </span>
